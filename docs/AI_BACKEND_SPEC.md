...
- 전체 문자열은 `question`의 200자 제한 안에 들어가야 하므로 최근 1~2개 문답만 짧게 보낸다.
...
- 200자를 넘는 대화 기록이 필요하면 이 방식 대신 별도 대화 이력 필드를 추가하는 API 변경이 필요하다.
...
| --- | --- | --- | --- |
| `400` | `INVALID_REQUEST` | 요청 형식이나 값이 잘못됨 | 요청 데이터 수정 |
...
- `400`, `401`, `422`는 같은 요청을 그대로 재시도하지 않는다.
